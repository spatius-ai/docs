...
<Note>
**Only available in [Direct Mode](/direct-mode/overview)**. In LiveKit Agents Integration and Backend Mode integrations, the connection is managed by the LiveKit Agents worker or developer backend, and is not exposed through `AvatarController.ConnectionState`.
</Note>
