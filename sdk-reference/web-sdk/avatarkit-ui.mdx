...
npx shadcn@latest add https://ui.spatius.ai/r/spatius-avatar.json
```
...
| `connection` | `{ roomName: string; token: string; url: string }` | LiveKit connection details for the avatar motion stream. Required. |
...
| `customEndpoint` | `string` | Advanced override for the AvatarKit API endpoint. Leave unset for normal regional routing. |
| `drivingServiceMode` | `DrivingServiceMode` | AvatarKit driving mode. Defaults to `DrivingServiceMode.backend`. |
| `region` | `string` | AvatarKit region. Defaults to `'us-west'`. |
...
| `disconnect()` | `() => Promise<void>` | Disconnects the avatar motion stream and resets state to idle. |
