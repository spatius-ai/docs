...
<Note>
`@spatius/avatarkit-rtc@1.1.3` requires `@spatius/avatarkit@1.3.7` or newer.
</Note>

<Warning>
**Initialize the underlying SDK with `DrivingServiceMode.rtc`.** Constructing an `AvatarPlayer` throws if the SDK was initialized with another mode, or before `initialize()` was called.
...
1. It runs on top of the underlying SDK initialized with `DrivingServiceMode.rtc`, so the avatar accepts externally driven motion data.
