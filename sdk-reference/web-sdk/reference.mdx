...
`region` selects the Spatius deployment region. It defaults to `'us-west'`; supported values are `'us-west'` and `'ap-northeast'`. See [Regions](/api-reference/regions) for endpoint details and override options.
