...
Use `rtc` only when driving the avatar through `@spatius/avatarkit-rtc`. It unlocks no API and identifies RTC-driven sessions in reporting. The RTC Adapter requires it; see [RTC Adapter](/sdk-reference/web-sdk/rtc-adapter).
...
| Field | What it does |
|-------|--------------|
| `inputAudioFormat` | The codec of the audio you feed into the SDK via `send()` / `yieldAudioData()`. Fixed for the whole session; do not vary it per call. Direct Mode only. Under `'opus'` the configured `sampleRate` does not apply: Opus always decodes at 48 kHz, and the SDK reports that rate for the session. |
