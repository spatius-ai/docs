...
The Spatius Python SDK creates server-side avatar sessions over WebSocket. It handles session-token creation, WebSocket authentication, audio upload, motion frame payload callbacks, and optional egress to LiveKit or Agora.
...
The SDK defaults to the `us-west` region. `ap-northeast` is also supported. You only need to pass endpoint URLs when using custom infrastructure.
...
# 4. Receive encoded motion frame payloads through transport_frames
...
When configured with `livekit_egress`, audio and motion data are streamed to a LiveKit room instead of being returned through the WebSocket connection. For end-to-end integration details, see the [LiveKit Agent server side guide](/livekit-agents/server).
...
Receives encoded motion frame payloads from the server:
