...
- `inputAudioFormat`: The codec of the audio you feed into the SDK via `send`. `.pcm` (default) for raw PCM16, `.opus` for Opus. Fixed for the whole session; audio that does not match is reported as `AvatarError.invalidAudioInput`. Direct mode only.
...
| `rtc` | The avatar is driven through the companion RTC SDK, `AvatarKitRTC`. Set it only on that path. It unlocks no API and does not change runtime behavior; it identifies RTC-driven sessions in reporting. |
...
| `opus` | Opus. Decoded to PCM locally for rendering and forwarded upstream unchanged. Direct mode only. |
