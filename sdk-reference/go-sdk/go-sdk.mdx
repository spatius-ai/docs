...
The Go SDK creates Spatius avatar sessions from backend services. It requests a short-lived session token from the Console API, opens the ingress WebSocket, sends audio, and returns animation frames through callbacks.
...
`SendAudio` returns the request ID associated with the audio. The callback passed to `WithTransportFrames` receives animation frame payloads and a `last` flag.
...
- `WithTransportFrames(func([]byte, bool))` - receives animation frame payloads.
