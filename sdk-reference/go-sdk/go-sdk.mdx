...
The SDK defaults to `us-west`. `ap-northeast` is also supported. Passing a region is enough for normal production use:
...
The SDK composes endpoint URLs from the region:
...
Explicit endpoint URLs override `WithRegion`:
...
- `WithRegion(region string)` - region used to compose default endpoint URLs.
