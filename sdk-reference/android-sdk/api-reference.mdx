...
<Accordion title="Configuration(region, audioFormat, drivingServiceMode, logLevel)">
...
- `inputAudioFormat`: The codec of the audio you feed into the SDK via `send`. `AudioCodec.PCM` (default) for raw PCM16, `AudioCodec.OPUS` for Opus. Fixed for the whole session; audio that does not match is reported as `AvatarError.InvalidAudioInput`. Direct mode only.
...
| `RTC` | The avatar is driven through the companion RTC SDK, `ai.spatius:avatarkit-rtc`. Set it only on that path. It unlocks no API and does not change runtime behavior; it identifies RTC-driven sessions in reporting. |
