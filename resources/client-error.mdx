...
| `unsupportedAvatarAsset` | The avatar asset requires a newer SDK. | Upgrade the SDK to a version that supports the asset. |
...
| `invalidAudioInput` | Audio does not match the declared `inputAudioFormat`. | Under `'opus'`, send mono Ogg Opus or one bare Opus packet per call, keeping one shape for the whole conversation. The error message names the specific problem. |
...
| `failedToDownloadAvatarAssets` | Failed to download avatar assets. | Check network connectivity and retry. |
| `invalidAudioInput(message)` | Audio does not match the declared `inputAudioFormat`. | Under `.opus`, send mono Opus and keep one shape for the whole conversation. The error message names the specific problem. |
...
| `IncompatibleAvatarAsset` | Avatar asset is incompatible with the SDK. | Upgrade the SDK or regenerate the avatar assets. |
| `FailedToDownloadAvatarAssets` | Failed to download avatar assets. | Check network connectivity and retry. |
| `InvalidAudioInput(message)` | Audio does not match the declared `inputAudioFormat`. | Under `AudioCodec.OPUS`, send mono Opus and keep one shape for the whole conversation. The error message names the specific problem. |
| `InvalidAnimationData` | Motion data payload is invalid. | Verify your Backend Mode or RTC data path. |
