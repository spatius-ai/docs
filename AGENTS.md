...
- **API Reference** → the public REST Open API for programmatic Avatar creation. Hand-written pages live in `open-api/` (overview, authentication, errors); endpoint pages are auto-generated from `openapi/avatar-open-api.json` via the group-level `openapi` key in `docs.json`.
