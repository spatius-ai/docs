...
Initialize AvatarKit with `DrivingServiceMode.rtc`. `AvatarPlayer` throws if the SDK was initialized with another mode, or before `initialize()` was called.
...
`AvatarKitRTC` expects a standard AvatarKit setup with an initialized SDK, a loaded avatar, and an `AvatarView`. Initialize AvatarKit with `drivingServiceMode: .rtc`; constructing `AvatarPlayer` in another mode, or before `initialize`, traps.
...
#### Using your own Agora engine

If your app already owns and manages an `AgoraRtcEngineKit`, hand it to the player with `attach(to:)` instead of calling `connect(_:)`. The SDK borrows the engine to read avatar data from the encoded video stream and render it. It does not create, join, or destroy the engine, and `detach()` returns it untouched.

```swift
let engine = AgoraRtcEngineKit.sharedEngine(with: config, delegate: self)

try player.attach(to: engine)   // before joinChannel

engine.setClientRole(.audience)
// subscribe to encoded video, then:
engine.joinChannel(byToken: token, channelId: channel, uid: uid, mediaOptions: options)

// Later:
await player.detach()
```

`attach(to:)` must be called before `joinChannel`, and the two paths are exclusive: do not mix it with `connect(_:)`. It requires an `AgoraProvider`, and throws if the player is already connected.
...
The Android package expects a standard AvatarKit setup with an initialized SDK, a loaded avatar, and an `AvatarView`. Initialize AvatarKit with `drivingServiceMode = DrivingServiceMode.RTC`; constructing `AvatarPlayer` in another mode, or before `initialize()`, throws `IllegalStateException`.
...
AvatarSDK.initialize(
    context = context,
    appId = appId,
    configuration = Configuration(drivingServiceMode = DrivingServiceMode.RTC),
)
