...
```
Your Client → Your Backend → Spatius Console API → Session Token (24 h max)
```

The Session Token must be set before `start()`. See the [Session token API](/api-reference/api-reference) for backend implementation.
</Note>
