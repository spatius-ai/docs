...
The Spatius API lets your backend create and manage avatars, and generate downloadable videos from an existing Avatar and audio.

| API group | Input | Result |
| --- | --- | --- |
| Avatar API | Source image URL | A custom Avatar for SDK integrations. |
| [Video API](/api-reference/video-generation) | Avatar ID and audio URL | An asynchronous job and a downloadable MP4. |
...
Each Avatar creation job reserves one Avatar Creation when it is accepted. The reservation is captured when preprocessing completes, and released if the job fails before that point. When the balance is insufficient, `POST /avatars` is rejected with `402` and code `insufficient_avatar_units`.
