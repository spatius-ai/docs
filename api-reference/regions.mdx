...
| Region | Notes |
| --- | --- |
| `us-west` | Default for the Go Server SDK and current server examples; fallback for automatic selection. |
| `ap-northeast` | Asia Pacific Northeast region. |
| `cn-beijing` | China Beijing region. |

## Endpoint URLs

You normally do not need to set endpoint URLs explicitly. AvatarKit clients and the Python Server SDK select a region automatically when none is provided. The Go Server SDK defaults to `us-west`; integration setup pages state each plugin's default.
