...
| --- | --- | --- |
| Audio (WAV) | `audio/wav`, `audio/x-wav`, `audio/wave`, `audio/vnd.wave` | 500 MiB |
...
Keep source URLs accessible for the preparation window of up to **30 minutes**; each transfer has a **five-minute** deadline. Only WAV audio inputs are supported. Empty files and `application/octet-stream` are not supported. Audio decoding and duration checks happen asynchronously, so acceptance does not guarantee a successful render.
...
The Console API checks video access and request rates. It does not impose an active-video-job quota; the video service manages execution concurrency. Back off after `429` responses. The Video API does not reserve Avatar Creations; video generation consumes **50 credits per minute**. See [Video Generation Pricing](/pricing/video-generation) for credit estimates.
